...
async function loadCoinData() {
    ...
    if (!coinId || !availableCoins.includes(coinId)) {
        displayError("コインIDが無効か、指定されていません。");
        return;
    }
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`評価ファイルが見つかりません: ${coinId}.json`);
        }
        ...
        displaySupportedExchanges(); // DEXの表示を追加
...
function displayPageTitle(data) {
    document.title = `${data.name || 'プロジェクト'} (${(data.symbol || '').toUpperCase()}) 分析 | Crypto Analysis Hub`;
}
...
function displayRelatedPosts(posts) {
    ...
    if (!posts || posts.length === 0) {
        container.innerHTML = '<p>関連する分析記事はありません。</p>';
        return;
    }
    let html = '';
    posts.forEach(post => {
